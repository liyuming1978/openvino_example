...
public:
    explicit CTCGreedyDecoderImpl(const CNNLayer* layer): ExtLayerBase(layer) {
        try {
            if (cnnLayer.insData.empty() || cnnLayer.outData.size() != 1)
                THROW_IE_EXCEPTION << "Incorrect number of input/output edges!";

            std::vector<DataConfigurator> inps;
            for (const auto &in : cnnLayer.insData)
                inps.emplace_back(ConfLayout::PLN);
            addConfig(inps, {DataConfigurator(ConfLayout::PLN)});
