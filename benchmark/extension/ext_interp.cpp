...
public:
    explicit InterpImpl(const CNNLayer* layer): ExtLayerBase(layer) {
        try {
            if (cnnLayer.insData.size() != 1 || cnnLayer.outData.empty())
                THROW_IE_EXCEPTION << "Incorrect number of input/output edges!";

            if (cnnLayer.insData[0].lock()->dims.size() != 4)
            ...
            pad_beg = cnnLayer.GetParamAsInt("pad_beg");
            pad_end = cnnLayer.GetParamAsInt("pad_end");
            ...
            addConfig({DataConfigurator(blk_layout)}, {DataConfigurator(blk_layout)});
    ...
    StatusCode execute(std::vector<Blob::Ptr>& inputs, std::vector<Blob::Ptr>& outputs,
        ...
        int IC = static_cast<int>(inputs[0]->getTensorDesc().getDims()[1]);
