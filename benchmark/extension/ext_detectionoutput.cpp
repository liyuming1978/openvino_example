...
public:
    explicit DetectionOutputImpl(const CNNLayer* layer) : ExtLayerBase(layer) {
        try {
            ...
            _num_priors = static_cast<int>(cnnLayer.insData[idx_priors].lock()->dims[0] / _prior_size);

            if (_num_priors * _num_loc_classes * 4 != cnnLayer.insData[0].lock()->dims[0])
                THROW_IE_EXCEPTION << "Number of priors must match number of location predictions.";

            if (_num_priors * _num_classes != cnnLayer.insData[idx_confidence].lock()->dims[0])
                THROW_IE_EXCEPTION << "Number of priors must match number of confidence predictions.";

            _num = static_cast<int>(cnnLayer.insData[idx_confidence].lock()->getTensorDesc().getDims()[0]);
            ...
            InferenceEngine::SizeVector conf_size = cnnLayer.insData[idx_confidence].lock()->dims;
            ...
            addConfig({DataConfigurator(ConfLayout::PLN),
