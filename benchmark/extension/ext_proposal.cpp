...
void generate_anchors(int base_size, float* ratios,
    ...
    const float center = 0.5f * (base_size - 1.0f);

    // enumerate all transformed boxes
    for (int ratio = 0; ratio < num_ratios; ++ratio) {
        // transformed width & height for given ratio factors
        const float ratio_w = std::roundf(std::sqrt(base_area / ratios[ratio]));
        const float ratio_h = std::roundf(ratio_w * ratios[ratio]);
        ...
        for (int scale = 0; scale < num_scales; ++scale) {
            ...
            const float scale_w = 0.5f * (ratio_w * scales[scale] - 1.0f);
            const float scale_h = 0.5f * (ratio_h * scales[scale] - 1.0f);
    ...
    for (int h = 0; h < bottom_H; ++h) {
        for (int w = 0; w < bottom_W; ++w) {
            ...
            for (int anchor = 0; anchor < num_anchors; ++anchor) {
                const float dx = p_box[(anchor * 4 + 0) * bottom_area];
                const float dy = p_box[(anchor * 4 + 1) * bottom_area];

                const float d_log_w = p_box[(anchor * 4 + 2) * bottom_area];
                const float d_log_h = p_box[(anchor * 4 + 3) * bottom_area];
                ...
                // width & height of box
                const float ww = x1 - x0 + 1.0f;
                const float hh = y1 - y0 + 1.0f;
                ...
                x0 = std::max<float>(0.0f, std::min<float>(x0, img_W - 1.0f));
                y0 = std::max<float>(0.0f, std::min<float>(y0, img_H - 1.0f));
                x1 = std::max<float>(0.0f, std::min<float>(x1, img_W - 1.0f));
                y1 = std::max<float>(0.0f, std::min<float>(y1, img_H - 1.0f));

                // recompute new width & height
                const float box_w = x1 - x0 + 1.0f;
                const float box_h = y1 - y0 + 1.0f;
...
void nms_cpu(const int num_boxes, int is_dead[],
    ...
    __m256  vc_fone = _mm256_set1_ps(1.0f);
    ...
    for (int box = 0; box < num_boxes; ++box) {
        ...
        for (; tail < num_boxes; ++tail) {
            ...
            if (x0i <= x1j && y0i <= y1j && x0j <= x1i && y0j <= y1i) {
                ...
                // intersection area
                const float width  = std::max<float>(0.0f,  x1 - x0 + 1.0f);
                const float height = std::max<float>(0.0f,  y1 - y0 + 1.0f);
                const float area   = width * height;

                // area of A, B
                const float A_area = (x1i - x0i + 1.0f) * (y1i - y0i + 1.0f);
                const float B_area = (x1j - x0j + 1.0f) * (y1j - y0j + 1.0f);
...
public:
    explicit ProposalImpl(const CNNLayer *layer): ExtLayerBase(layer) {
        try {
            if (cnnLayer.insData.size() != 3 || cnnLayer.outData.size() != 1)
                THROW_IE_EXCEPTION << "Incorrect number of input/output edges!";
            feat_stride_ = static_cast<size_t>(cnnLayer.GetParamAsInt("feat_stride"));
            base_size_ = static_cast<size_t>(cnnLayer.GetParamAsInt("base_size"));
            min_size_ = static_cast<size_t>(cnnLayer.GetParamAsInt("min_size"));
            pre_nms_topn_ = cnnLayer.GetParamAsInt("pre_nms_topn");
            post_nms_topn_ = cnnLayer.GetParamAsInt("post_nms_topn");
            nms_thresh_ = cnnLayer.GetParamAsFloat("nms_thresh");
            scales = cnnLayer.GetParamAsFloats("scale", {});
            ratios = cnnLayer.GetParamAsFloats("ratio", {});
    ...
    StatusCode execute(std::vector<Blob::Ptr> &inputs, std::vector<Blob::Ptr> &outputs,
        ...
        for (int n = 0; n < nn; ++n) {
            ...
            nms_cpu(pre_nms_topn, &is_dead[0], &unpacked_boxes[0], &roi_indices_[0], &num_rois, 0, nms_thresh_, post_nms_topn_);
