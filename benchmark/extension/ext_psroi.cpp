...
public:
    explicit PSROIPoolingImpl(const CNNLayer* layer): ExtLayerBase(layer) {
        try {
            if (cnnLayer.insData.size() != 2 || cnnLayer.outData.size() != 1)
                THROW_IE_EXCEPTION << "Incorrect number of input/output edges!";
            // LayerSetUp
            output_dim_ = static_cast<size_t>(cnnLayer.GetParamAsInt("output_dim"));
            group_size_ = static_cast<size_t>(cnnLayer.GetParamAsInt("group_size"));
            spatial_scale_ = cnnLayer.GetParamAsFloat("spatial_scale");
            pooled_height_ = group_size_;
            pooled_width_ = group_size_;

            SizeVector inDims = cnnLayer.insData[0].lock()->getTensorDesc().getDims();
            channels = static_cast<int>(inDims[1]);
            height = static_cast<int>(inDims[2]);
            width = static_cast<int>(inDims[3]);

            SizeVector outDims = cnnLayer.outData[0]->getTensorDesc().getDims();
