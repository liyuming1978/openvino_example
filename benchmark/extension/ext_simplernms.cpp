...
public:
    explicit SimplerNMSImpl(const CNNLayer *layer): ExtLayerBase(layer) {
        try {
            if (cnnLayer.insData.size() != 3 || cnnLayer.outData.size() != 1)
                THROW_IE_EXCEPTION << "Incorrect number of input/output edges!";

            min_box_size_ = cnnLayer.GetParamAsInt("min_bbox_size");
            feat_stride_ = cnnLayer.GetParamAsInt("feat_stride");
            pre_nms_topn_ = cnnLayer.GetParamAsInt("pre_nms_topn");
            post_nms_topn_ = cnnLayer.GetParamAsInt("post_nms_topn");
            iou_threshold_ = cnnLayer.GetParamAsFloat("iou_threshold");
            ...
            if (cnnLayer.outData[0]->getTensorDesc().getDims().size() != 2 ||
                    cnnLayer.insData[0].lock()->getTensorDesc().getDims().size() != 4)
