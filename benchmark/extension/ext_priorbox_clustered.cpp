...
public:
    explicit PriorBoxClusteredImpl(const CNNLayer* layer): ExtLayerBase(layer) {
        try {
            if (cnnLayer.insData.size() != 2 || cnnLayer.outData.empty())
                THROW_IE_EXCEPTION << "Incorrect number of input/output edges!";

            widths_ = cnnLayer.GetParamAsFloats("width", {});
            heights_ = cnnLayer.GetParamAsFloats("height", {});
            clip_ = cnnLayer.GetParamAsInt("clip");
            variance_ = cnnLayer.GetParamAsFloats("variance", {});
            img_h_ = cnnLayer.GetParamAsInt("img_h", 0);
            img_w_ = cnnLayer.GetParamAsInt("img_w", 0);
            step_ = cnnLayer.GetParamAsFloat("step", 0);
            step_h_ = cnnLayer.GetParamAsFloat("step_h", 0);
            step_w_ = cnnLayer.GetParamAsFloat("step_w", 0);
            offset_ = cnnLayer.GetParamAsFloat("offset");

            addConfig({{ConfLayout::PLN, true}, {ConfLayout::PLN, true}}, {{ConfLayout::PLN, true}});
