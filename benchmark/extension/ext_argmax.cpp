...
public:
    explicit ArgMaxImpl(const CNNLayer* layer): ExtLayerBase(layer) {
        try {
            if (cnnLayer.insData.size() != 1 || cnnLayer.outData.empty())
                THROW_IE_EXCEPTION << "Incorrect number of input/output edges!";

            out_max_val_ = static_cast<bool>(cnnLayer.GetParamAsInt("out_max_val"));
            top_k_       = cnnLayer.GetParamAsInt("top_k");

            has_axis_ = (cnnLayer.params.find("axis") != cnnLayer.params.end());
            axis_index_ = has_axis_ ?
                                std::stoi(cnnLayer.params["axis"]) :0;
