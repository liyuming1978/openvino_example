...
public:
    explicit PriorBoxImpl(const CNNLayer *layer): ExtLayerBase(layer) {
        try {
            if (cnnLayer.insData.size() != 2 || cnnLayer.outData.empty())
                THROW_IE_EXCEPTION << "Incorrect number of input/output edges!";
            _offset = cnnLayer.GetParamAsFloat("offset");
            _step = cnnLayer.GetParamAsFloat("step", 0);
            _min_sizes = cnnLayer.GetParamAsFloats("min_size", {});
            _max_sizes = cnnLayer.GetParamAsFloats("max_size", {});
            _flip = static_cast<bool>(cnnLayer.GetParamAsInt("flip"));
            _clip = static_cast<bool>(cnnLayer.GetParamAsInt("clip"));
            ...
            const std::vector<float> variance = cnnLayer.GetParamAsFloats("variance", {});
            ...
            for (int w = 0; w < W; ++w) {
                for (size_t msIdx = 0; msIdx < _min_sizes.size(); msIdx++) {
                    ...
                    for (float ar : _aspect_ratios) {
                        ...
                        box_width = _min_sizes[msIdx] * sqrt(ar);
                        box_height = _min_sizes[msIdx] / sqrt(ar);
