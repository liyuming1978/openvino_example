...
public:
    explicit NormalizeImpl(const CNNLayer* layer): ExtLayerBase(layer) {
        try {
            if (cnnLayer.insData.size() != 1 || cnnLayer.outData.size() != 1)
            ...
            across_spatial = static_cast<bool>(cnnLayer.GetParamAsInt("across_spatial"));
            channel_shared = static_cast<bool>(cnnLayer.GetParamAsInt("channel_shared"));
            eps = cnnLayer.GetParamAsFloat("eps");

            addConfig({{ConfLayout::PLN, false, 0}}, {{ConfLayout::PLN, false, 0}}, true);
...
class NormalizeFactory : public ImplFactory<NormalizeImpl> {
public:
    explicit NormalizeFactory(const CNNLayer *layer): ImplFactory(layer) {}
...
REG_FACTORY_FOR(NormalizeFactory, Normalize);
